...
class PandaTowerBimanualEnv(BimanualTaskEnv):
    ...
    def __init__(self, render: bool = False, num_blocks: int = 1, control_type: str = "ee", reward_type = 'sparse') -> None:
        ...
        robot0 = Panda(sim, index=0,block_gripper=False, base_position=np.array([-0.8, 0.0, 0.0]), control_type=control_type, base_orientation = [0,0,0,1])
        robot1 = Panda(sim, index=1, block_gripper=False, base_position=np.array([0.8, 0.0, 0.0]),control_type=control_type, base_orientation = [0,0,1,0])
