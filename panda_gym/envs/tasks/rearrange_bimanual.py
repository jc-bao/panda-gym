...
class RearrangeBimanual(Task):
  def __init__(
    ...
    with self.sim.no_rendering():
      ...
      self.sim.place_visualizer(target_position=np.zeros(
        3), distance=1.2, yaw=45, pitch=-30)
    # gym env parameters
  ...
  def _create_scene(self) -> None:
    ...
    self.sim.create_table(length=1., width=1, height=0.4,
                          x_offset=(-table_x), index=0)
    self.sim.create_table(length=1., width=1, height=0.4,
                          x_offset=(table_x), index=1)
  ...
  def reset(self, attr_dict={}) -> None:
    ...
    if not 'goal' in attr_dict.keys():
      ...
      if 'num_need_handover' in attr_dict.keys():
        ...
        num_need_handover = self.np_random.choice(
          a=self.num_blocks+1, size=1, p=self.os_num_dist)[0]
    ...
    for i in range(self.num_blocks):
      ...
      self.sim.set_base_pose(
        "object"+str(i), self.obj_pos[i*3:(i+1)*3], np.array([0.0, 0.0, 0.0, 1.0]))
    # set tables
    table_x = 0.5 + self.gap_distance/2
    self.sim.set_base_pose("table0", np.array(
      [-table_x, 0, -0.2]), [1, 0, 0, 0])
    self.sim.set_base_pose("table1", np.array(
      [table_x, 0, -0.2]), [1, 0, 0, 0])
    assert len(
      self.obj_pos) % self.num_blocks == 0, 'task observation shape linear to num blocks'
    return self.get_obs()

  def _sample_goal(self, num_need_handover) -> np.ndarray:
    goals = []
    handover_idx = self.np_random.choice(
  ...
  def _sample_objects(self) -> np.ndarray:
    ...
    if self.np_random.uniform() < self.obj_in_hand_rate:
      ...
      block_x_shift = self.np_random.uniform(
        low=-self.block_size[0]/3, high=self.block_size[0]/3)
  ...
  def compute_reward(self, achieved_goal, desired_goal, info: Dict[str, Any]) -> Union[np.ndarray, float]:
    ...
    rew = -np.mean(dist_block2goal > self.distance_threshold,
                   axis=-1, dtype=float)
    if len(rew) == 1:
      return rew[0]
